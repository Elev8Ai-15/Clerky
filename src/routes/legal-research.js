...
import { searchCaseLaw, searchDockets, lookupCitation, verifyCitations, getOpinionText, searchJudges, getCitationNetwork, checkApiHealth } from '../agents/legal-research';
import { getLitigationAnalytics, formatAnalyticsMarkdown } from '../agents/lex-machina';
import { badRequest, rateLimit } from '../utils/shared';
const app = new Hono();
// ── 1. CASE LAW SEARCH ───────────────────────────────────────
...
app.get('/citation', async (c) => {
    ...
    if (!cite)
        return badRequest(c, ['Query parameter "cite" is required']);
...
app.get('/opinion/:clusterId', async (c) => {
    ...
    if (!result)
        return c.json({ error: 'Opinion not found' }, 404);
    return c.json(result);
});
// ── 7. JUDGE SEARCH ──────────────────────────────────────────
...
app.get('/citations/:clusterId', async (c) => {
    ...
    if (isNaN(clusterId) || clusterId < 1)
        return badRequest(c, ['Invalid cluster ID']);
    const direction = (c.req.query('direction') || 'cited_by');
...
app.post('/quick', async (c) => {
    try {
        ...
        if (typeof query !== 'string' || query.length > 500)
            return badRequest(c, ['Query must be a string <= 500 characters']);
