...
calendar.get('/', async (c) => {
    ...
    return c.json({ events: result.results, page, page_size: pageSize, total: coalesceInt(totalRow?.total) });
});
...
calendar.post('/', async (c) => {
    ...
    `).bind(safe.title, safe.description || null, safe.event_type, safe.case_id || null, safe.organizer_id || 1, safe.location || null, safe.virtual_link || null, safe.start_datetime, safe.end_datetime, safe.all_day || 0, safe.color || '#3B82F6', safe.reminder_minutes || 30, safe.attendees || null, safe.notes || null).run();
        await auditLog(c.env.DB, 'create', 'calendar_events', result.meta.last_row_id, 1, { title: safe.title, event_type: safe.event_type });
...
calendar.put('/:id', async (c) => {
    try {
        const id = c.req.param('id');
        if (!(await checkExists(c.env.DB, 'calendar_events', id)))
            return notFound(c, 'Event');
        const body = await c.req.json();
        if (body.event_type && !VALID_EVENT_TYPES.includes(body.event_type))
            return badRequest(c, [`Invalid event_type: ${body.event_type}`]);
