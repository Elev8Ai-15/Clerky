...
tasks.get('/', async (c) => {
  ...
  if (caseId) { query += ' AND t.case_id = ?'; countQuery += ' AND case_id = ?'; params.push(caseId); countParams.push(caseId) }
  if (assignedTo) { query += ' AND t.assigned_to = ?'; countQuery += ' AND assigned_to = ?'; params.push(assignedTo); countParams.push(assignedTo) }
  ...
  return c.json({ tasks: result.results, page, page_size: pageSize, total: coalesceInt((totalRow as any)?.total) })
})
...
tasks.post('/', async (c) => {
  try {
    ...
    await auditLog(c.env.DB, 'create', 'tasks_deadlines', result.meta.last_row_id as number, 1, { title: safe.title, assigned_to: safe.assigned_to })
...
tasks.put('/:id', async (c) => {
  try {
    ...
    if (!(await checkExists(c.env.DB, 'tasks_deadlines', id))) return notFound(c, 'Task')

    const body = await c.req.json()
    if (body.priority && !VALID_PRIORITIES.includes(body.priority)) return badRequest(c, [`Invalid priority: ${body.priority}`])
    if (body.status && !VALID_STATUSES.includes(body.status)) return badRequest(c, [`Invalid status: ${body.status}`])
    if (body.task_type && !VALID_TYPES.includes(body.task_type)) return badRequest(c, [`Invalid task_type: ${body.task_type}`])
