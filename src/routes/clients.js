...
clients.get('/', async (c) => {
    ...
    return c.json({ clients: result.results, page, page_size: pageSize, total: coalesceInt(totalRow?.total) });
});
...
clients.post('/', async (c) => {
    ...
    `).bind(safe.first_name, safe.last_name, safe.email || null, safe.phone || null, safe.address || null, safe.city || null, safe.state || null, safe.zip_code || null, safe.client_type || 'individual', safe.status || 'active', safe.company_name || null, safe.assigned_attorney_id || null, safe.notes || null).run();
        await auditLog(c.env.DB, 'create', 'clients', result.meta.last_row_id, 1, { first_name: safe.first_name, last_name: safe.last_name });
...
clients.put('/:id', async (c) => {
    try {
        const id = c.req.param('id');
        if (!(await checkExists(c.env.DB, 'clients', id)))
            return notFound(c, 'Client');
        const body = await c.req.json();
        // Validate enum fields if present
        if (body.client_type && !VALID_TYPES.includes(body.client_type))
            return badRequest(c, [`Invalid client_type: ${body.client_type}`]);
