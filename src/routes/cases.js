...
cases.get('/', async (c) => {
  ...
  `;
    ...
    if (status) {
        query += ' AND cm.status = ?';
        countQuery += ' AND cm.status = ?';
        params.push(status);
        countParams.push(status);
    }
    if (type) {
        query += ' AND cm.case_type = ?';
        countQuery += ' AND cm.case_type = ?';
        params.push(type);
        countParams.push(type);
    }
    if (attorney) {
        query += ' AND cm.lead_attorney_id = ?';
        countQuery += ' AND cm.lead_attorney_id = ?';
        params.push(attorney);
        countParams.push(attorney);
    }
    if (search) {
        query += ' AND (cm.title LIKE ? OR cm.case_number LIKE ?)';
        countQuery += ' AND (cm.title LIKE ? OR cm.case_number LIKE ?)';
    ...
    return c.json({ cases: result.results, page, page_size: pageSize, total: coalesceInt(totalRow?.total) });
});
...
cases.post('/', async (c) => {
    try {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).bind(caseNumber, safe.title, safe.description || null, safe.case_type, safe.status || 'open', safe.priority || 'medium', safe.client_id, safe.lead_attorney_id, safe.court_name || null, safe.opposing_counsel || null, safe.opposing_party || null, safe.date_filed || new Date().toISOString().split('T')[0], safe.estimated_value || null, safe.retainer_amount || null).run();
        await auditLog(c.env.DB, 'create', 'cases_matters', result.meta.last_row_id, 1, { case_number: caseNumber, title: safe.title, case_type: safe.case_type });
...
cases.put('/:id', async (c) => {
    try {
        const id = c.req.param('id');
        if (!(await checkExists(c.env.DB, 'cases_matters', id)))
            return notFound(c, 'Case');
        const body = await c.req.json();
        const safe = sanitize(body);
        const fields = [];
        ...
        for (const field of allowedFields) {
            if (safe[field] !== undefined) {
                if (field === 'case_type' && !VALID_TYPES.includes(safe[field]))
                    return badRequest(c, [`Invalid case_type: ${safe[field]}`]);
                if (field === 'status' && !VALID_STATUSES.includes(safe[field]))
                    return badRequest(c, [`Invalid status: ${safe[field]}`]);
                if (field === 'priority' && !VALID_PRIORITIES.includes(safe[field]))
                    return badRequest(c, [`Invalid priority: ${safe[field]}`]);
                fields.push(`${field} = ?`);
