...
users.get('/:id', async (c) => {
  ...
  return c.json({ user, active_cases: coalesceInt((activeCases as any)?.count), upcoming_tasks: recentTasks.results })
})
...
users.post('/', async (c) => {
  try {
    ...
    await auditLog(c.env.DB, 'create', 'users_attorneys', result.meta.last_row_id as number, 1, { email: safe.email, full_name: safe.full_name })
...
users.put('/:id', async (c) => {
  try {
    const id = c.req.param('id')
    if (!(await checkExists(c.env.DB, 'users_attorneys', id))) return notFound(c, 'User')

    const body = await c.req.json()
    if (body.role && !VALID_ROLES.includes(body.role)) return badRequest(c, [`Invalid role: ${body.role}`])
    ...
    for (const f of allowed) {
      if (safe[f] !== undefined) { fields.push(`${f} = ?`); values.push(safe[f]) }
    }
