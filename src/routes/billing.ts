...
billing.get('/stats', async (c) => {
  ...
  return c.json({
    total_revenue: coalesceInt((totalRev as any)?.total),
    outstanding: coalesceInt((outstanding as any)?.total),
    overdue_amount: coalesceInt((overdue as any)?.total),
    overdue_count: coalesceInt((overdue as any)?.count),
    monthly_billable: coalesceInt((monthly as any)?.total)
  })
})
...
billing.get('/invoices', async (c) => {
  ...
  if (status) { query += ' AND bi.status = ?'; countQuery += ' AND status = ?'; params.push(status); countParams.push(status) }
  if (clientId) { query += ' AND bi.client_id = ?'; countQuery += ' AND client_id = ?'; params.push(clientId); countParams.push(clientId) }
  ...
  return c.json({ invoices: result.results, page, page_size: pageSize, total: coalesceInt((totalRow as any)?.total) })
})
...
billing.post('/invoices', async (c) => {
  try {
    ...
    await auditLog(c.env.DB, 'create', 'billing_invoices', result.meta.last_row_id as number, 1, { invoice_number: invoiceNum, client_id: safe.client_id, total_amount: safe.total_amount })
...
billing.put('/invoices/:id', async (c) => {
  try {
    const id = c.req.param('id')
    if (!(await checkExists(c.env.DB, 'billing_invoices', id))) return notFound(c, 'Invoice')

    const body = await c.req.json()
    if (body.status && !VALID_INVOICE_STATUSES.includes(body.status)) return badRequest(c, [`Invalid status: ${body.status}`])
...
billing.get('/time-entries', async (c) => {
  ...
  if (caseId) { query += ' AND te.case_id = ?'; countQuery += ' AND case_id = ?'; params.push(caseId); countParams.push(caseId) }
  if (userId) { query += ' AND te.user_id = ?'; countQuery += ' AND user_id = ?'; params.push(userId); countParams.push(userId) }
  ...
  return c.json({ time_entries: result.results, page, page_size: pageSize, total: coalesceInt((totalRow as any)?.total) })
})
...
billing.post('/time-entries', async (c) => {
  try {
    ...
    await auditLog(c.env.DB, 'create', 'time_entries', result.meta.last_row_id as number, 1, { case_id: safe.case_id, hours: safe.hours })
