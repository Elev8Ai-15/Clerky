...
export class LexMachinaClient {
    ...
    async queryDistrictCases(query) {
        if (!this.isEnabled)
            return [];
        try {
    ...
    // Get case details
    async getCaseDetails(caseIds) {
        if (!this.isEnabled || caseIds.length === 0)
            return [];
        try {
            const results = [];
...
export function getLitigationAnalytics(jurisdiction, caseType, lmClient) {
    ...
    const ct = caseType?.toLowerCase()
        ...
        || 'personal_injury';
    if (isKS && KS_LITIGATION_STATS[ct])
        return KS_LITIGATION_STATS[ct];
    if (isMO && MO_LITIGATION_STATS[ct])
        return MO_LITIGATION_STATS[ct];
    // Default to PI stats for unknown case types
    if (isKS)
        return KS_LITIGATION_STATS['personal_injury'];
    if (isMO)
        return MO_LITIGATION_STATS['personal_injury'];
...
export function formatAnalyticsMarkdown(analytics) {
