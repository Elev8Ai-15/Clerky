...
export function getCourtIds(jurisdiction) {
    const j = jurisdiction?.toLowerCase()?.trim() || '';
    if (j === 'kansas' || j === 'ks')
        return ['kan', 'kansctapp', 'ca10'];
    if (j === 'missouri' || j === 'mo')
        return ['mod', 'moed', 'moctapp', 'mo', 'ca8'];
    if (j === 'federal' || j === 'us')
        return ['scotus', 'ca8', 'ca10', 'kan', 'mod', 'moed'];
    if (j === '8th circuit' || j === 'ca8' || j === '8th')
        return ['ca8'];
    if (j === '10th circuit' || j === 'ca10' || j === '10th')
        return ['ca10'];
    if (j === 'multi-state' || j === 'all' || j === '')
        return []; // empty = all jurisdictions
...
async function clFetch(url, token) {
...
export async function searchCaseLaw(opts) {
    const { query, jurisdiction = 'multi-state', date_filed_after, date_filed_before, cited_gt, status = 'Published', order_by = 'score desc', semantic = false, page_size = 20, token } = opts;
    ...
    try {
        ...
        if (courtIds.length > 0)
            params.set('court', courtIds.join(' '));
        if (date_filed_after)
            params.set('filed_after', date_filed_after);
        if (date_filed_before)
            params.set('filed_before', date_filed_before);
        if (cited_gt)
            params.set('cited_gt', String(cited_gt));
        if (status)
...
export async function searchDockets(opts) {
    const { query, jurisdiction = 'multi-state', date_filed_after, date_filed_before, nature_of_suit, page_size = 20, token } = opts;
    const courtIds = opts.court_ids || getCourtIds(jurisdiction);
    try {
        const params = new URLSearchParams();
        params.set('q', query);
        ...
        if (date_filed_before)
            params.set('filed_before', date_filed_before);
        if (nature_of_suit)
            params.set('suitNature', nature_of_suit);
        const url = `${CL_API}/search/?${params.toString()}`;
        const res = await clFetch(url, token);
        if (!res.ok) {
            return {
...
export async function lookupCitation(citation, token) {
    try {
        const params = new URLSearchParams();
...
export async function verifyCitations(citations, token) {
    const results = [];
...
export async function getOpinionText(clusterId, token) {
    try {
        // Get cluster info
        ...
        return {
            ...
            citations: (cluster.citations || []).map((c) => typeof c === 'string' ? c : c.cite || ''),
...
export async function getCitationNetwork(clusterId, direction, token) {
    try {
