...
import { searchCaseLaw } from './legal-research';
...
function detectResearchSubtype(msg) {
    const subtypes = [];
    const m = msg.toLowerCase();
    if (m.includes('statute') || m.includes('sol') || m.includes('limitation') || m.includes('deadline'))
        subtypes.push('statute_lookup');
    if (m.includes('case law') || m.includes('precedent') || m.includes('holding') || m.includes('ruling'))
        subtypes.push('case_law');
    if (m.includes('compare') || m.includes('strength') || m.includes('weakness') || m.includes('vs'))
        subtypes.push('comparison');
    if (m.includes('citation') || m.includes('cite') || m.includes('authority'))
        subtypes.push('citation_check');
    if (m.includes('rule') || m.includes('procedure') || m.includes('evidence'))
        subtypes.push('procedural');
    if (m.includes('comparative fault') || m.includes('50% bar') || m.includes('pure comparative'))
        subtypes.push('comparative_fault');
    if (subtypes.length === 0)
        subtypes.push('general_research');
    return subtypes;
}
// ── Map case type to knowledge base keys ────────────────────
function getCaseTypeKeys(caseType, msg) {
    const keys = [];
    const m = msg.toLowerCase();
    // Direct type match
    if (caseType) {
        ...
        if (typeMap[caseType])
            keys.push(typeMap[caseType]);
    }
    // Message-based detection
    if (m.includes('personal injury') || m.includes('negligence') || m.includes('tort'))
        keys.push('personal_injury');
    if (m.includes('medical malpractice') || m.includes('med mal') || m.includes('malpractice') || m.includes('informed consent') || m.includes('screening panel'))
        keys.push('medical_malpractice');
    if (m.includes('employment') || m.includes('wrongful termination') || m.includes('discrimination'))
        keys.push('employment');
    if (m.includes('custody') || m.includes('divorce') || m.includes('alimony') || m.includes('family') || m.includes('maintenance'))
        keys.push('family');
    if (m.includes('corporate') || m.includes('series a') || m.includes('merger') || m.includes('shareholder') || m.includes('llc'))
        keys.push('corporate');
    if (m.includes('contract')) {
        keys.push('contract_written');
        keys.push('contract_oral');
    }
    if (m.includes('fraud'))
        keys.push('fraud');
    if (m.includes('death') || m.includes('wrongful death'))
        keys.push('wrongful_death');
    if (m.includes('sovereign') || m.includes('government') || m.includes('tort claim'))
        keys.push('sovereign_immunity');
    if (m.includes('comparative fault') || m.includes('50% bar') || m.includes('pure comparative') || m.includes('fault'))
        keys.push('comparative_fault');
    if (m.includes('workers comp') || m.includes('work injury') || m.includes('on the job'))
        keys.push('workers_comp');
    if (m.includes('product') || m.includes('defect') || m.includes('manufacturer'))
        keys.push('product_liability');
    if (m.includes('premises') || m.includes('slip') || m.includes('fall') || m.includes('property owner'))
        keys.push('premises_liability');
    if (m.includes('insurance') || m.includes('bad faith') || m.includes('denied claim') || m.includes('vexatious'))
        keys.push('insurance_bad_faith');
    if (m.includes('consumer') || m.includes('deceptive') || m.includes('unfair practice') || m.includes('kcpa') || m.includes('mmpa'))
        keys.push('consumer_protection');
    if (m.includes('section 1983') || m.includes('civil rights') || m.includes('qualified immunity') || m.includes('§ 1983'))
        keys.push('government_immunity_federal');
    if (m.includes('real estate') || m.includes('deed') || m.includes('title') || m.includes('adverse possession') || m.includes('recording') || m.includes('conveyance'))
        keys.push('real_estate');
    return [...new Set(keys)];
}
...
export async function runResearcher(input, llm, mem0Context) {
    ...
    if (totalStatutes > 0 || totalCases > 0 || liveResults.length > 0) {
        ...
        if (liveResults.length > 0)
            content += `, and **${liveResults.length} live case result(s)** from CourtListener`;
        content += ` for this query.`;
        if (input.matter.case_id)
            content += ` Analysis is contextualized to ${input.matter.case_number}.`;
        content += `\n\n`;
        if (liveSearchStatus === 'live')
            content += `> 🟢 **Live legal research active** — connected to CourtListener (Free Law Project) for real-time case law across all US jurisdictions.\n\n`;
    }
    else {
    ...
    if (jx === 'federal') {
        ...
        if (subtypes.includes('statute_lookup'))
            content += `- **FRCP Rule 6(a)** — exclude day of event; 3 days added for service by mail\n`;
    }
    ...
    if (liveResults.length > 0) {
        ...
        for (const r of liveResults.slice(0, 8)) {
            ...
            if (r.snippet)
                content += `- **Excerpt:** ${r.snippet.substring(0, 300)}${r.snippet.length > 300 ? '...' : ''}\n`;
            content += `\n`;
        }
    ...
    if (totalCases > 0)
        actions.push(`Review ${totalCases} case(s) for distinguishing facts`);
    if (input.matter.case_id && !input.matter.statute_of_limitations)
        actions.push('URGENT: Calculate and calendar SOL for this matter');
    actions.push('Prepare research memorandum for matter file');
    if (liveResults.length > 0)
        actions.push(`Review ${liveResults.length} additional live case(s) from CourtListener`);
