...
// ═══════════════════════════════════════════════════════════════
import { initMemoryTables, assembleMatterContext, getConversationHistory, writeMemory, trackSession } from './memory';
import { runResearcher } from './researcher';
import { runDrafter } from './drafter';
import { runAnalyst } from './analyst';
import { runStrategist } from './strategist';
import { createMem0Client } from './mem0';
...
function classifyIntent(message, history) {
    ...
    const scores = { researcher: 0, drafter: 0, analyst: 0, strategist: 0 };
    // ─── Researcher signals ───────────────────────────────────
    ...
    for (const k of researchKeywords) {
        if (msg.includes(k))
            scores.researcher += 3;
    }
    // Kansas-specific statutory patterns (KANSAS MODE: boosted priority)
    if (msg.match(/k\.?s\.?a\.?\s|kansas\sstatute|chapter\s60|10th\scircuit/i))
        scores.researcher += 6;
    // Missouri-specific statutory patterns (MISSOURI MODE: boosted priority)
    if (msg.match(/rsmo\s|r\.?s\.?mo\.?\s|missouri\sstatute|missouri\ssupreme\scourt\srule|8th\scircuit/i))
        scores.researcher += 6;
    // KS MODE: auto-boost for SOL and comparative fault queries
    if (msg.match(/sol\b|statute\s+of\s+limitation|60-513|2[- ]year/i))
        scores.researcher += 4;
    if (msg.match(/50%\s*bar|comparative\s+fault|60-258a|proportional\s+fault/i))
        scores.researcher += 4;
    // MO MODE: auto-boost for MO-specific SOL, pure comparative, J&S, ESI queries
    if (msg.match(/516\.120|5[- ]year\s+sol|five[- ]year/i))
        scores.researcher += 4;
    if (msg.match(/pure\s+comparative|537\.765|537\.067|joint\s+(and|&)\s+several/i))
        scores.researcher += 4;
    if (msg.match(/fact\s+plead|esi\b|proportionality|discovery\s+cost/i))
        scores.researcher += 3;
    // General legal research patterns
    if (msg.match(/what\s+(is|are)\s+the\s+(law|rule|statute|standard)/i))
        scores.researcher += 4;
    // ─── Drafter signals ──────────────────────────────────────
    ...
    for (const k of draftKeywords) {
        if (msg.includes(k))
            scores.drafter += 3;
    }
    if (msg.match(/draft\s+(a|the|my)\s/i))
        scores.drafter += 5;
    if (msg.match(/motion\s+to\s+(dismiss|compel|strike|suppress)/i))
        scores.drafter += 6;
    // ─── Analyst signals ──────────────────────────────────────
    ...
    for (const k of analystKeywords) {
        if (msg.includes(k))
            scores.analyst += 3;
    }
    if (msg.match(/risk\s+assess/i))
        scores.analyst += 5;
    if (msg.match(/strength.+weakness|weakness.+strength/i))
        scores.analyst += 5;
    if (msg.match(/what\s+am\s+i\s+missing/i))
        scores.analyst += 4;
    if (msg.match(/50%\s+bar|pure\s+comparative|comparative\s+fault/i))
        scores.analyst += 4;
    // ─── Strategist signals ───────────────────────────────────
    ...
    for (const k of strategistKeywords) {
        if (msg.includes(k))
            scores.strategist += 3;
    }
    if (msg.match(/propose\s+\d+\s+/i))
        scores.strategist += 5;
    if (msg.match(/what\s+(should|can)\s+(i|we)\s+do/i))
        scores.strategist += 4;
    if (msg.match(/pros?\s+(and|&)\s+cons?/i))
        scores.strategist += 5;
    if (msg.match(/what\s+am\s+i\s+missing/i))
        scores.strategist += 5;
...
export async function orchestrate(db, message, sessionId, caseId, jurisdiction, userId = 1, env) {
    const startTime = Date.now();
    // 1. Initialize services
    await initMemoryTables(db);
    const mem0 = createMem0Client(env?.MEM0_API_KEY);
    const llm = createLLMClient(env?.OPENAI_API_KEY, env?.OPENAI_BASE_URL, env?.OPENAI_MODEL, env?.ANTHROPIC_API_KEY);
    ...
    switch (route.agent) {
        case 'researcher':
            result = await runResearcher(agentInput, llm, mem0Context);
            break;
        case 'drafter':
            result = await runDrafter(agentInput, llm, mem0Context);
            break;
        case 'analyst':
            result = await runAnalyst(agentInput, llm, mem0Context);
            break;
        case 'strategist':
            result = await runStrategist(agentInput, llm, mem0Context);
            break;
        default:
            result = await runStrategist(agentInput, llm, mem0Context);
            break;
    ...
    if (route.sub_agents && route.sub_agents.length > 0) {
        for (const sub of route.sub_agents) {
            ...
            switch (sub) {
                case 'researcher':
                    subResult = await runResearcher(agentInput, llm, mem0Context);
                    break;
                case 'drafter':
                    subResult = await runDrafter(agentInput, llm, mem0Context);
                    break;
                case 'analyst':
                    subResult = await runAnalyst(agentInput, llm, mem0Context);
                    break;
                case 'strategist':
                    subResult = await runStrategist(agentInput, llm, mem0Context);
                    break;
            }
            if (subResult) {
                ...
                for (const c of subResult.citations) {
                    if (!result.citations.find(rc => rc.reference === c.reference))
                        result.citations.push(c);
                }
                // Deduplicate risks
                for (const r of subResult.risks_flagged) {
                    if (!result.risks_flagged.includes(r))
                        result.risks_flagged.push(r);
                }
                result.memory_updates.push(...subResult.memory_updates);
                // Append sub-agent summary
                const agentEmoji = { researcher: '🔍', drafter: '📝', analyst: '🧠', strategist: '🎯' };
    ...
    // 11. Track session
    try {
        await trackSession(db, sessionId, caseId, result.agent_type, result.tokens_used);
    }
    catch (e) { /* non-critical */ }
    // 12. Final timing
...
function buildRoutingHeader(route, mem0Loaded) {
    const agentEmoji = {
