...
function detectDocType(msg) {
    const m = msg.toLowerCase();
    if (m.includes('demand letter') || m.includes('demand'))
        return 'demand_letter';
    if (m.includes('motion to dismiss') || m.includes('dismiss'))
        return 'motion_dismiss';
    if (m.includes('motion to compel') || m.includes('compel discovery'))
        return 'motion_compel';
    if (m.includes('engagement letter') || m.includes('retainer'))
        return 'engagement_letter';
    if (m.includes('complaint') || m.includes('petition'))
        return 'complaint';
    if (m.includes('summary judgment') || m.includes('msj'))
        return 'summary_judgment';
    if (m.includes('discovery response') || m.includes('interrogator') || m.includes('production'))
...
export async function runDrafter(input, llm, mem0Context) {
    ...
    if (input.matter.case_id) {
        ...
        if (input.matter.description)
            content += `- Matter description: ${input.matter.description}\n`;
        if (input.matter.date_filed)
            content += `- Case filed: ${input.matter.date_filed}\n`;
        if (input.matter.estimated_value)
            content += `- Estimated value: $${Number(input.matter.estimated_value).toLocaleString()}\n`;
        if (input.matter.opposing_counsel)
            content += `- Opposing counsel: ${input.matter.opposing_counsel}\n`;
