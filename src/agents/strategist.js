...
function detectStrategyType(msg) {
    const subtypes = [];
    const m = msg.toLowerCase();
    if (m.includes('settlement') || m.includes('settle') || m.includes('resolve'))
        subtypes.push('settlement');
    if (m.includes('timeline') || m.includes('calendar') || m.includes('schedule') || m.includes('deadline'))
        subtypes.push('timeline');
    if (m.includes('budget') || m.includes('cost') || m.includes('fee'))
        subtypes.push('budget');
    if (m.includes('mediat') || m.includes('arbitrat') || m.includes('adr'))
        subtypes.push('adr');
    if (m.includes('trial') || m.includes('hearing') || m.includes('jury'))
        subtypes.push('trial_prep');
    if (m.includes('missing') || m.includes('proactive') || m.includes('recommend') || m.includes('what am i'))
        subtypes.push('proactive');
    if (m.includes('scenario') || m.includes('option') || m.includes('strateg') || m.includes('plan'))
        subtypes.push('scenario_planning');
    if (m.includes('venue') || m.includes('forum') || m.includes('choice of law'))
        subtypes.push('venue_analysis');
...
export async function runStrategist(input, llm, mem0Context) {
    ...
    if (subtypes.includes('proactive')) {
        ...
        if (input.matter.case_id && !input.matter.statute_of_limitations) {
            ...
            if (isKS)
                content += `   - Kansas: Typically 2 years for PI (K.S.A. 60-513), 5 years written contract (K.S.A. 60-511)\n`;
            if (isMO)
                content += `   - Missouri: Typically 5 years for PI (RSMo § 516.120), 10 years written contract (RSMo § 516.110)\n`;
        ...
        if (overdue.length > 0) {
            ...
            for (const t of overdue)
                content += `   - "${t.title}" — was due ${t.due_date}\n`;
        ...
        if (isKS)
            content += `Kansas: Medical malpractice screening panel per K.S.A. 65-4901 may apply. `;
        if (isMO)
            content += `Missouri: Affidavit of merit per RSMo § 538.225 for med mal. `;
    ...
    if (subtypes.includes('settlement'))
        actions.push('Prepare settlement demand/proposal package');
    if (subtypes.includes('timeline'))
        actions.push('Generate detailed litigation timeline in calendar');
    if (subtypes.includes('budget'))
        actions.push('Present budget projection to client for approval');
