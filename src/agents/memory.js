...
export async function initMemoryTables(db) {
...
export async function assembleMatterContext(db, caseId, sessionId, mem0, userQuery, userId) {
    const base = {
  ...
  `).bind(caseId).first();
    ...
    base.documents = (docs.results || []);
    base.tasks = (tasks.results || []);
    base.recent_events = (events.results || []);
    const b = billing;
    ...
    base.prior_research = (research.results || []).map(r => ({ id: r.id, agent_type: r.agent_type, key: r.memory_key || r.key || '', value: r.memory_value || r.value || '', confidence: r.confidence, created_at: r.created_at }));
    base.prior_analysis = (analysis.results || []).map(r => ({ id: r.id, agent_type: r.agent_type, key: r.memory_key || r.key || '', value: r.memory_value || r.value || '', confidence: r.confidence, created_at: r.created_at }));
    return base;
}
// ─── Get conversation history ───────────────────────────────
export async function getConversationHistory(db, sessionId, limit = 20) {
    const result = await db.prepare(`SELECT role, content, agent_type, created_at FROM ai_chat_messages WHERE session_id = ? ORDER BY created_at DESC LIMIT ?`).bind(sessionId, limit).all();
    return (result.results || []).reverse();
}
// ─── Write agent memory (Mem0 primary, D1 fallback) ─────────
export async function writeMemory(db, updates, caseId, sessionId, mem0, userId, jurisdiction) {
...
export async function readMemory(db, caseId, agentType, key) {
    let query = `SELECT * FROM agent_memory WHERE case_id = ? AND agent_type = ?`;
    const params = [caseId, agentType];
    if (key) {
...
export async function searchMemory(db, query, caseId, mem0, userId) {
    // Try Mem0 first
    if (mem0?.isEnabled && userId) {
        try {
...
// ─── Track agent session ────────────────────────────────────
export async function trackSession(db, sessionId, caseId, agentType, tokensUsed) {
    const existing = await db.prepare(`SELECT * FROM agent_sessions WHERE session_id = ?`).bind(sessionId).first();
    if (existing) {
...
export function formatMatterContext(m) {
    if (!m.case_id)
...
Description: ${m.description || 'N/A'}`;
    ...
    if (m.prior_research.length > 0) {
        ...
        for (const r of m.prior_research.slice(0, 3))
            ctx += `\n- [${r.key || 'note'}] ${(r.value || '').substring(0, 200)}...`;
    }
    if (m.prior_analysis.length > 0) {
        ctx += `\n\n── Prior Analysis Notes ──`;
        for (const a of m.prior_analysis.slice(0, 3))
